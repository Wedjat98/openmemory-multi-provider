...
class LLMConfig(BaseModel):
    ...
    base_url: Optional[str] = Field(None, description="Base URL for OpenAI-compatible API (e.g., https://api.openai.com/v1)")
    ollama_base_url: Optional[str] = Field(None, description="Base URL for Ollama server (e.g., http://host.docker.internal:11434)")
    anthropic_api_key: Optional[str] = Field(None, description="Anthropic API key or 'env:ANTHROPIC_API_KEY'")
    together_api_key: Optional[str] = Field(None, description="Together API key or 'env:TOGETHER_API_KEY'")
    groq_api_key: Optional[str] = Field(None, description="Groq API key or 'env:GROQ_API_KEY'")
    mistral_api_key: Optional[str] = Field(None, description="Mistral API key or 'env:MISTRAL_API_KEY'")
    google_api_key: Optional[str] = Field(None, description="Google API key or 'env:GOOGLE_API_KEY'")
    deepseek_api_key: Optional[str] = Field(None, description="DeepSeek API key or 'env:DEEPSEEK_API_KEY'")
    xai_api_key: Optional[str] = Field(None, description="xAI API key or 'env:XAI_API_KEY'")
    litellm_api_key: Optional[str] = Field(None, description="LiteLLM API key or 'env:LITELLM_API_KEY'")
    langchain_api_key: Optional[str] = Field(None, description="LangChain API key or 'env:LANGCHAIN_API_KEY'")
...
async def get_supported_providers():
    """Get list of supported LLM and embedder providers."""
    return {
        "llm_providers": [
            {
                "name": "openai",
                "description": "OpenAI API and OpenAI-compatible APIs",
            ...
            },
            {
                ...
                "required_fields": ["model", "together_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "groq",
                "description": "Groq API",
                "required_fields": ["model", "groq_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "mistral",
                "description": "Mistral AI API",
                "required_fields": ["model", "mistral_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "google",
                "description": "Google AI/Gemini API",
                "required_fields": ["model", "google_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "deepseek",
                "description": "DeepSeek API",
                "required_fields": ["model", "deepseek_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "xai",
                "description": "xAI API",
                "required_fields": ["model", "xai_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "litellm",
                "description": "LiteLLM API",
                "required_fields": ["model", "litellm_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            },
            {
                "name": "langchain",
                "description": "LangChain API",
                "required_fields": ["model", "langchain_api_key"],
                "optional_fields": ["base_url", "temperature", "max_tokens"]
            }
        ],
        "embedder_providers": [
            {
                "name": "openai",
                "description": "OpenAI Embeddings API and OpenAI-compatible APIs",
                "required_fields": ["model", "api_key"],
            ...
            {
                "name": "anthropic",
                "description": "Anthropic Claude API (for embeddings)",
                "required_fields": ["model", "anthropic_api_key"],
                "optional_fields": []
            },
            {
                "name": "together",
                "description": "Together AI API (for embeddings)",
                "required_fields": ["model", "together_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "groq",
                "description": "Groq API (for embeddings)",
                "required_fields": ["model", "groq_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "mistral",
                "description": "Mistral AI API (for embeddings)",
                "required_fields": ["model", "mistral_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "google",
                "description": "Google AI/Gemini API (for embeddings)",
                "required_fields": ["model", "google_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "deepseek",
                "description": "DeepSeek API (for embeddings)",
                "required_fields": ["model", "deepseek_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "xai",
                "description": "xAI API (for embeddings)",
                "required_fields": ["model", "xai_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "litellm",
                "description": "LiteLLM API (for embeddings)",
                "required_fields": ["model", "litellm_api_key"],
                "optional_fields": ["base_url"]
            },
            {
                "name": "langchain",
                "description": "LangChain API (for embeddings)",
                "required_fields": ["model", "langchain_api_key"],
                "optional_fields": ["base_url"]
            }
        ]

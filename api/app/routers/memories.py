...
async def list_memories(
    ...
    for item in memories:
        if check_memory_access_permissions(db, item, app_id):
            ...
            memory_response = MemoryResponse(
                ...
                app_id=item.app_id,
                app_name=item.app.name if item.app else "Unknown",
                categories=[cat.name for cat in item.categories] if item.categories else [],
    ...
    return Page(
        ...
        page=params.page,
        size=params.size,
        pages=(len(filtered_items) + params.size - 1) // params.size
    )

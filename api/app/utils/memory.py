...
def _validate_provider_config(provider: str, config: dict) -> bool:
    """
    Validate provider configuration based on the provider type.
    
    Args:
        provider: The provider name (e.g., 'openai', 'ollama', 'anthropic')
    ...
    if provider == "openai":
        ...
        # Ollama requires model and ollama_base_url
        required_fields = ["model", "ollama_base_url"]
        return all(field in config for field in required_fields)
    
    elif provider == "anthropic":
        # Anthropic requires model and anthropic_api_key
        required_fields = ["model", "anthropic_api_key"]
        return all(field in config for field in required_fields)
    
    elif provider in ["together", "groq", "mistral", "google", "deepseek", "xai", "litellm", "langchain"]:
        # These providers require model and their specific API key
        api_key_field = f"{provider}_api_key"
        required_fields = ["model", api_key_field]
        return all(field in config for field in required_fields)
    
    else:
        print(f"Warning: Unknown provider '{provider}', skipping validation")
        return True


def _get_embedding_dimensions(provider: str, model: str) -> int:
    """
    Get the expected embedding dimensions for a given provider and model.
    ...
    """
    # BGE models typically use 768 dimensions
    if "bge" in model.lower():
        return 768
    
    # OpenAI models typically use 1536 dimensions
    if provider == "openai":
    ...
    # Ollama models - check common patterns
    if provider == "ollama":
        if "bge" in model.lower():
            return 768
...
def _normalize_provider_config(provider: str, config: dict) -> dict:
    """
    Normalize provider configuration to ensure it has the correct structure for Mem0.
    ...
    if provider == "openai":
        # For OpenAI, handle base_url through environment variable
        if "base_url" in config and config["base_url"]:
            # Set the base_url as an environment variable that Mem0 can use
            import os
            os.environ["OPENAI_BASE_URL"] = config["base_url"]
            print(f"Set OPENAI_BASE_URL environment variable to: {config['base_url']}")
        # Remove base_url from config as it's not directly supported
        if "base_url" in normalized_config:
            del normalized_config["base_url"]
    
    elif provider == "ollama":
        # For Ollama, ensure we have ollama_base_url
        if "ollama_base_url" in config and config["ollama_base_url"]:
            normalized_config["ollama_base_url"] = config["ollama_base_url"]
        else:
            normalized_config["ollama_base_url"] = "http://host.docker.internal:11434"
    
    elif provider == "anthropic":
        # For Anthropic, use the anthropic_api_key as api_key
        if "anthropic_api_key" in config:
            normalized_config["api_key"] = config["anthropic_api_key"]
    
    elif provider in ["together", "groq", "mistral", "google", "deepseek", "xai", "litellm", "langchain"]:
        # For other providers, use their specific API key as api_key
        api_key_field = f"{provider}_api_key"
        if api_key_field in config:
            normalized_config["api_key"] = config[api_key_field]
        
        # Handle base_url through environment variable for other providers
        if "base_url" in config and config["base_url"]:
            import os
            env_var_name = f"{provider.upper()}_BASE_URL"
            os.environ[env_var_name] = config["base_url"]
            print(f"Set {env_var_name} environment variable to: {config['base_url']}")
        # Remove base_url from config as it's not directly supported
        if "base_url" in normalized_config:
            del normalized_config["base_url"]
            ...
            if db_config:
                ...
                if "mem0" in json_config:
                    ...
                    if "llm" in mem0_config and mem0_config["llm"] is not None:
                        llm_provider = mem0_config["llm"].get("provider", "openai")
                        llm_config = mem0_config["llm"].get("config", {})
                        
                        # Validate LLM configuration
                        if not _validate_provider_config(llm_provider, llm_config):
                            print(f"Warning: Invalid LLM configuration for provider '{llm_provider}', using defaults")
                        else:
                            # Normalize the configuration
                            normalized_llm_config = _normalize_provider_config(llm_provider, llm_config)
                            
                            # Fix Ollama URLs for Docker if needed
                            if llm_provider == "ollama":
                                normalized_llm_config = _fix_ollama_urls({"config": normalized_llm_config})["config"]
                            
                            config["llm"] = {
                                "provider": llm_provider,
                                "config": normalized_llm_config
                            }
                    
                    # Update Embedder configuration if available
                    if "embedder" in mem0_config and mem0_config["embedder"] is not None:
                        embedder_provider = mem0_config["embedder"].get("provider", "openai")
                        embedder_config = mem0_config["embedder"].get("config", {})
                        
                        # Validate Embedder configuration
                        if not _validate_provider_config(embedder_provider, embedder_config):
                            print(f"Warning: Invalid Embedder configuration for provider '{embedder_provider}', using defaults")
                        else:
                            # Normalize the configuration
                            normalized_embedder_config = _normalize_provider_config(embedder_provider, embedder_config)
                            
                            # Fix Ollama URLs for Docker if needed
                            if embedder_provider == "ollama":
                                normalized_embedder_config = _fix_ollama_urls({"config": normalized_embedder_config})["config"]
                            
                            config["embedder"] = {
                                "provider": embedder_provider,
                                "config": normalized_embedder_config
                            }
                            
                            # Update vector store dimensions based on embedder
                            embedder_model = normalized_embedder_config.get("model", "")
                            embedding_dims = _get_embedding_dimensions(embedder_provider, embedder_model)
                            if "vector_store" in config and "config" in config["vector_store"]:
                                ...
                                print(f"Updated vector store embedding dimensions to {embedding_dims} for {embedder_provider} model {embedder_model}")
